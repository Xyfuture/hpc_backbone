...
from models.big_lama.model.utils import norm_img, norm_batch_img, pad_batch_img_to_modulo, resize_batch_img
...
class LaMa:
    ...
    def _pad_forward(self, images: List[np.ndarray], masks: List[np.ndarray], config: Config) -> List[np.ndarray]:
        origin_heights = [image.shape[0] for image in images]
        origin_widths = [image.shape[1] for image in images]

        pad_batch_image = pad_batch_img_to_modulo(images, mod=self.pad_mod, square=self.pad_to_square,
                                                  min_size=self.min_size)

        pad_batch_mask = pad_batch_img_to_modulo(masks, mod=self.pad_mod, square=self.pad_to_square,
                                                 min_size=self.min_size)

        logger.info(f"final forward pad size: {pad_batch_image[0].shape}")

        results = self.forward(pad_batch_image, pad_batch_mask, config)
    ...
    def forward(self, batch_image, batch_mask, config: Config)->List[np.ndarray]:
    ...
    @torch.no_grad()
    def __call__(self, images, masks, config: Config):
        """
        ...
        reshape_images = resize_batch_img(images,config.hd_strategy_resize_limit)
        reshape_masks = resize_batch_img(masks,config.hd_strategy_resize_limit)

        results = self._pad_forward(reshape_images,reshape_masks,config)
